...
# ---------------------------------------------------------------------------#
    ...
    async def cmd_start(u: Update, c: ContextTypes.DEFAULT_TYPE):
        """Отправляет краткую справку и инструкции."""
        text = f"""*End‑of‑Life Bot 🇷🇺*

Я показываю состояние поддержки версий ПО по данным [endoflife.date](https://endoflife.date/).
...
• В инлайн‑режиме: `@{c.bot.username} nodejs` — получите статус прямо в другом чате.
...
"""
    ...
    async def cmd_help(u: Update, c: ContextTypes.DEFAULT_TYPE):
        ...
        text = f"""*Подробная справка*

Бот обращается к публичному API https://endoflife.date, чтобы показать сроки поддержки (EOL) разных продуктов.

📋 *Форматы ввода*
• `nodejs 20` — продукт + версия  
• `python` — только продукт (покажу состояние последней стабильной версии)  
• `- docker\n- kubernetes\n- postgres` — маркированный список; вы получите отдельное сообщение для каждого пункта.

🤖 *Inline*
В любом чате наберите `@{c.bot.username} <продукт>` — появятся подсказки.
